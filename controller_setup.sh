...
ssh -o StrictHostKeyChecking=no -i ${private_ssh_key_path} ${username}@${controller_node} "
  ...
  nohup kubectl port-forward svc/nginx-web-server 8080:8080 -n social-network > port-forward-nginx.log 2>&1 &
  nohup kubectl port-forward svc/media-frontend 8081:8081 -n social-network > port-forward-media.log 2>&1 &
